...
public class BitmapUtils {
 ...
 // 如果是放大图片，filter决定是否平滑，如果是缩小图片，filter无影响，我们这里是缩小图片，所以直接设置为false
    ...
    public static Bitmap decodeSampledBitmapFromBitmap(byte[] src, int reqWidth, int reqHeight)
    {
        final BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeByteArray(src,0,src.length);
        options.inSampleSize = calculateInSampleSize(options, reqWidth, reqHeight);
        // 使用获取到的inSampleSize值再次解析图片
        options.inJustDecodeBounds = false;
        Bitmap srcbitmap = BitmapFactory.decodeByteArray(src,0,src.length,options);
        return createScaleBitmap(srcbitmap,reqWidth,reqHeight,options.inSampleSize);
